...
class CoreDataStore {
    typealias CoreDataCompletionClosure = ((Any)->Swift.Void)
       ...
       static var storeName = "ArticleView"
    ...
    static var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = Bundle(for: CoreDataStore.self).url(forResource: storeName, withExtension: modelExtension)! // type your database name here..
        return NSManagedObjectModel(contentsOf: modelURL)!
    }()
    
    lazy var entities: [String]? = {
        if  CoreDataStore.managedObjectModel.entities.count > 0 {
            return CoreDataStore.managedObjectModel.entities.map { $0.name ?? "" }
        }
        return nil
    }()
    
    static var persistentStoreURL: URL? {
        let storeName = "\(CoreDataStore.storeName).\(CoreDataStore.storeFileExtension)"
        return ProjectUtils.documentsURL(with: storeName)
    }
    ...
    static var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        let url = persistentStoreURL // type your database name here...
